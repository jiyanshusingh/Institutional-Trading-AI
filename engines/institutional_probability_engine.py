"""
Institutional Probability Engine — 8-factor dual scoring (0–100 each side)

Each factor returns a dict with *bullish* and *bearish* scores.

Factors:
  1. Market Regime   (15/15) — NIFTY EMA alignment + swing structure + breadth
  2. Sector Strength  (15/15) — Relative performance + volume ratio
  3. Price Action     (20/20) — Swing structure + breakout/breakdown + support quality
  4. Volume           (15/15) — RVOL on up/down bars
  5. Breakout/Breakdown Quality (15/15) — A+ checklist for each direction
  6. Risk/Reward      (10/10) — Market-based RR (not constant)
  7. Indicators        (5/5)  — EMA + RSI + MACD + VWAP (both sides)
  8. Catalyst          (5/5)  — Accumulation / distribution
"""
...
class InstitutionalProbabilityEngine:
    ...
    def _score_market_regime(
        ...
        if nifty_df is not None and len(nifty_df) >= 60:
            ...
            # Price vs EMAs (bullish: above, bearish: below)
            if last_close > ema20:
                bull += 3
                detail["ema20"] = "above"
            ...
            # Swing structure (bullish: HH/HL, bearish: LL/LH)
        ...
        return {"bullish": min(bull, 12), "bearish": min(bear, 12), "max": 12, "detail": detail}
    ...
    def _score_risk_reward(self, df: pd.DataFrame) -> dict:
        ...
        risk_short = atr_val * self.sl_mult
        if swings["last_swing_high"] > 0 and swings["last_swing_high"] > entry:
            risk_short = max(risk_short, swings["last_swing_high"] - entry)
        supp, _ = _nearest_support(df["low"].values, entry)
        reward_short = atr_val * self.tp_mult
        if supp is not None and entry > supp:
            reward_short = min(reward_short, entry - supp)
