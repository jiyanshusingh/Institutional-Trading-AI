...
INTRADAY_LAST_BARS = 5       # skip entries in last N bars of the day
...
BROKERAGE_PER_TRADE = 20.0   # ₹20 flat per executed order
STT_PCT = 0.1                # 0.1 % STT on sell side
...
def fetch_data(
    ...
    *,
    ...
    # Resolve Upstox instrument key (needed for today intraday fetch)
    instr_key = None
    if provider_type == "upstox" and symbol.endswith(".NS"):
...
def decide_trade(
    ...
    *,
    ...
    for tc in strategy_result.trade_candidates:
        ...
        if tc.direction == "LONG" and score < MIN_PROB:
            return None
...
class WalkForwardBacktest:
    ...
    def _compute_costs(entry_price: float, exit_price: float, direction: str,
        ...
        total = slippage_entry + slippage_exit + brokerage + stt
        return {
    ...
    def run(self, days: int = 120) -> BacktestSummary:
        ...
        # NIFTY intraday at the same timeframe as the stock
        ...
        # NIFTY daily: load the FULL available history (cache has ~5yr) so the
        # point-in-time slice (current_date) always has enough lookback for the
        # 200-bar EMA in market_regime. Truncating to `days` would starve the
        # signal at the start of the run.
        ...
        if self._nifty_intraday is not None:
            min_len = min(len(df), len(self._nifty_intraday))
            df = df.iloc[:min_len].reset_index(drop=True)
            self._nifty_intraday = self._nifty_intraday.iloc[:min_len].reset_index(drop=True)
        ...
        total_slices = len(df) - WINDOW_SIZE
        for i in range(WINDOW_SIZE, len(df)):
            window_df = df.iloc[:i].tail(WINDOW_SIZE).reset_index(drop=True)
            ...
            try:
                ...
                try:
                    if self._nifty_intraday is not None and i < len(self._nifty_intraday):
                        ...
                        nifty_win = self._nifty_intraday.iloc[:i].tail(WINDOW_SIZE).reset_index(drop=True)
                ...
                if decision is not None:
                    ...
                    elif self._position_size_for(bt.entry_price, bt.stop_loss) <= 0.0:
                        ...
                        try:
                            ...
                            for _btc in _bres.trade_candidates:
                                ...
                                if _btc.entry_price is None or _btc.stop_loss is None or _btc.take_profit is None:
                                    continue
                                if _btc.ranking_score < MIN_PROB:
                                    continue
            ...
            if self.intraday_mode and i in last_of_day:
                for ot in list(open_trades):
                    ...
                    pos_size = self._position_size_for(ot.entry_price, ot.stop_loss)
            ...
            # ── Stale trade cleanup ─────────────────────────────
            stale = [ot for ot in open_trades if i - ot.entry_idx > MAX_HOLD_BARS]
            for s in stale:
                s.exit_idx = i
                ...
                pos_size = self._position_size_for(s.entry_price, s.stop_loss)
    ...
    def _position_size_for(self, entry: float, sl: float) -> float:
        """Delegates to the shared capital model (scripts.capital_model)."""
        return position_size_for(entry, sl)
    ...
    def _settle_trade(self, ot: BacktestTrade, result: dict):
        ...
        if risk > 0 and ot.entry_price > 0:
            if result["result"] == "WIN":
                gross_return = abs(ot.exit_price - ot.entry_price)
                ot.r_multiple = gross_return / risk
            else:
                ot.r_multiple = -1.0
        else:
            ot.r_multiple = 0.0
    ...
    def _check_exit(self, trade: BacktestTrade, df: pd.DataFrame, current_idx: int) -> dict | None:
        ...
        for j in range(entry_idx + 1, min(current_idx + 1, entry_idx + MAX_HOLD_BARS + 1)):
            if j >= len(df):
                break
            candle = df.iloc[j]
